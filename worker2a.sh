...
User = postgres
ExecStart = /usr/local/bin/pg_autoctl create worker --monitor "postgresql://autoctl_node@monitor/pg_auto_failover" --citus-cluster one --name worker2a --group 3 --ssl-self-signed --auth trust --run
Restart = always
StartLimitBurst = 0
